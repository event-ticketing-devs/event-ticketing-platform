...
export default function ReportVenueModal({ open, venueId, venueName, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiClient.post("/venue-reports", {
        venueId,
        reason,
      ...
      if (error.response?.data?.message === "You have already reported this venue") {
        toast.error("You have already reported this venue");
      } else {
      ...
      <div className="bg-bg-primary border border-border rounded-lg max-w-md w-full max-h-screen overflow-y-auto">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-text-primary">Report Venue</h3>
            <button
              onClick={handleClose}
          ...
          <div className="mb-4">
            <p className="text-sm text-text-secondary">
              You are reporting: <span className="font-medium text-text-primary">{venueName}</span>
            </p>
          </div>
            ...
            <div className="bg-warning/10 border border-warning/30 rounded-lg p-3">
              <div className="flex">
                ...
                <div>
                  ...
                  <p className="text-sm text-text-secondary">
                    Please only report venues that violate our community guidelines.
                  </p>
                </div>
              </div>
            </div>
