...
const OrganizerContacts = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'in-progress': return 'bg-slate-100 text-slate-900 border-slate-200';
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600">Manage contact inquiries and team communication</p>
        </div>
          ...
          <div className="flex border-b border-slate-200">
            <button
              ...
              className={`flex-1 px-6 py-4 text-sm font-medium transition-colors ${
                activeTab === 'inquiries'
                  ? 'text-slate-900 border-b-2 border-slate-900 bg-slate-100'
        ...
        {activeTab === 'inquiries' ? (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              {[
                { label: 'Total Messages', value: pagination.total, color: 'bg-slate-100', iconColor: 'text-slate-900' },
                { 
                  label: 'Pending', 
                ...
                <div key={index} className="bg-white border border-slate-200 p-6">
                  <div className="flex items-center">
                    <div className={`w-12 h-12 ${stat.color} border border-slate-200 flex items-center justify-center mr-4`}>
            ...
            <div className="flex gap-2">
              {['all', 'pending', 'in-progress', 'resolved', 'closed'].map((status) => (
                <button
                  ...
                  className={`px-3 py-2 border border-slate-200 text-sm font-medium transition-colors ${
                    filter === status
        ...
        <div className="bg-white border border-slate-200">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="w-8 h-8 border-b-2 rounded-full border-slate-900 border-t-transparent animate-spin"></div>
            </div>
          ) : contacts.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-slate-500">No contact messages match your current filter.</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-slate-50 border-b border-slate-200">
                    <tr>
                      <th className="text-left py-3 px-6 text-xs font-medium text-slate-500 uppercase tracking-wider">Contact</th>
                      <th className="text-left py-3 px-6 text-xs font-medium text-slate-500 uppercase tracking-wider">Event</th>
                      <th className="text-left py-3 px-6 text-xs font-medium text-slate-500 uppercase tracking-wider">Subject</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {contacts.map((contact) => (
                      <tr key={contact._id} className="hover:bg-slate-50">
                        ...
                        <td className="py-4 px-6">
                          <div>
                            <p className="font-medium text-slate-900">{contact.eventId?.title}</p>
                        ...
                        <td className="py-4 px-6">
                          <button
                            ...
                            className="text-slate-900 border border-slate-300 hover:border-slate-900 p-2 font-medium text-sm"
                          >
                            View & Respond
                          </button>
                        </td>
                ...
                <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-1 border border-slate-300 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Previous
                    </button>
          ...
          <div className="space-y-6">
            ...
            {myEvents.length > 0 && (
              <div className="bg-white border border-slate-200 p-6">
                ...
                <select
                  ...
                  className="w-full md:w-1/2 px-4 py-3 border border-slate-200 focus:ring-1 focus:ring-slate-900 focus:border-slate-900"
                >
                  {myEvents.map((event) => (
            ...
            {selectedEvent ? (
              ...
              <div className="bg-white border border-slate-200 p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-slate-900 mb-2">No Events Available</h3>
                <p className="text-slate-600">You don't have any events to chat about yet.</p>
              </div>
            )}
          </div>
        )}
...
const OrganizerContactModal = ({ contact, onClose, onUpdateStatus }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50  flex items-center justify-center p-4 z-50">
      <div className="bg-white border-2 border-slate-200  max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="bg-slate-900 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-white">Event Inquiry</h2>
            <p className="text-slate-300 text-sm">{contact.eventId?.title}</p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 bg-slate-900 text-white border border-slate-200 hover:bg-slate-800 transition-colors text-sm"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                ...
                {contact.phone && (
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 bg-green-600 text-white border border-slate-200 hover:bg-green-700 transition-colors text-sm"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-slate-100 border border-slate-200 p-4">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="text-slate-900">{contact.eventId?.title}</p>
              </div>
              <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-slate-200 focus:ring-1 focus:ring-slate-900 focus:border-slate-900"
              >
            ...
            <div className="flex space-x-4 pt-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className={`flex-1 py-3 px-6 border border-slate-200 font-semibold text-white transition-colors ${
                  loading
